...
const TaskFormCard = ({ task, index , tasks,setTasks}) => {
  ...
  const handleOngoing = id => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.patch(`http://localhost:5000/tasks/ongoing/${id}`)
        .then((res) => {
          if (res.data.modifiedCount > 0) {
            axios.get("http://localhost:5000/tasks")
            .then(res => {
              const filteredTask = res.data.filter(task=> task.email===user?.email)
              setTasks(filteredTask)
            });
  ...
  const handleComplete = id => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.patch(`http://localhost:5000/tasks/complete/${id}`)
        .then((res) => {
          if (res.data.modifiedCount > 0) {
            axios.get("http://localhost:5000/tasks")
            .then(res => {
          ...
          <div className="card-body ">
           ...
           <p>{task?.priority}</p>
            ...
            <div className="card-actions items-center">
              <Link to={`/update/${task?._id}`}><button><FaEdit size={26} /></button></Link>
              <button onClick={()=>handleDelete(task?._id)}><FaTrashAlt size={26} /></button>
            </div>
            <div className="space-x-3">
              <button onClick={()=>handleOngoing(task?._id)} title="Ongoing"><FaArrowAltCircleRight size={26} /></button>
               <button  onClick={()=>handleComplete(task?._id)} title="Completed"><FaArrowAltCircleRight size={26} /></button>
            </div>
            </div>
          </div>
        </div>
