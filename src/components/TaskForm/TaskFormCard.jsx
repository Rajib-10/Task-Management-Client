  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://task-management-server-side-blush.vercel.app/tasks/${id}`
          )
          .then((res) => {
  ...
  const handleOngoing = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .patch(
            `https://task-management-server-side-blush.vercel.app/tasks/ongoing/${id}`
          )
          .then((res) => {
  ...
  const handleComplete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .patch(
            `https://task-management-server-side-blush.vercel.app/tasks/complete/${id}`
          )
          .then((res) => {
            if (res.data.modifiedCount > 0) {
              axios
